            ...
            try:
                ...
                for edge_num, edge in enumerate(edges):
                    ...
                    try:
                        ...
                        # For the first edge, add the start vertex with its orientation info
                        if edge_num == 1:
                        ...
                        # Always add the end vertex with its orientation info
                        vertex_chain.append(end_coords)
                        vertex_orientations.append(f"E{edge_num}:V-{vertex_end_orient_str[:3]}")
                        print(f"              Added end vertex with orientation {vertex_end_orient_str}")
                        ...
                        try:
                            ...
                            if edge_num == 1:
                                ...
                                if v1_tuple == last_tuple:
                                    ...
                                    print(f"              Connected via vertex1, added vertex2")
                                elif v2_tuple == last_tuple:
                                    vertex_chain.append(v1_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{v1_orient_str[:3]}")
                                    print(f"              Connected via vertex2, added vertex1")
                                else:
                                    vertex_chain.extend([v1_coords, v2_coords])
                                    vertex_orientations.extend([f"E{edge_num}:V-{v1_orient_str[:3]}", f"E{edge_num}:V-{v2_orient_str[:3]}"])
                                    print(f"              No connection found, added both vertices")
...
def plot_arrays_visualization(array_A, array_B, array_C, unit_projection_normal):
    ...
    for i, poly_data in enumerate(array_C):
        try:
            ...
            name = poly_data['name']
            if polygon.geom_type == 'Polygon' and polygon.area > 0:
                plot_polygon(polygon, ax3, facecolor='none', edgecolor='black', alpha=1.0, linewidth=0.7, linestyle='--', label=f'C: {name}', outline_only=True)
        except Exception as e:
