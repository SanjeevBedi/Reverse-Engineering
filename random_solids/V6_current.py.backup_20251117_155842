...
def extract_polygon_faces_from_connectivity(selected_vertices, merged_conn, tolerance=1e-6):
    ...
    # Step 4: Find all vertices within each face range
    print("\n[POLY FORM] Step 4: Finding vertices within each face range")
    ...
    for face_idx, face_eq in enumerate(unique_faces):
        ...
        for v_idx in range(N):
            ...
            # Check if vertex lies within plane range
            # Substitute vertex into both bounding plane equations
            dist_P = np.dot(face_eq['normal_P'], vertex) + face_eq['d_P']
            dist_M = np.dot(face_eq['normal_M'], vertex) + face_eq['d_M']
            
            # Vertex is in range if signs are opposite or either is zero
            # This means vertex lies between the two bounding planes
            if dist_P * dist_M <= 0:
                vertices_on_face.append(v_idx)
        
        face_eq['vertices_on_face'] = vertices_on_face
        # Convert to 1-indexed for display
        vertices_display = [v+1 for v in vertices_on_face]
        print(f"[POLY FORM] F{face_idx+1:2d}: {len(vertices_on_face):2d} vertices: {vertices_display}")
    
    # Step 4.4: Assign unassigned vertices to faces
    print("\n[POLY FORM] Step 4.4: Assigning vertices to ensure all have 2+ faces")
    ...
    # Find vertices that belong to fewer than 2 faces
    unassigned_vertices = [v_idx for v_idx in range(N)
                           if vertex_face_count[v_idx] < 2]
    
    print(f"[POLY FORM]   Found {len(unassigned_vertices)} vertices "
          f"assigned to fewer than 2 faces")
    ...
    for v_idx in unassigned_vertices:
        ...
        # Compute distance from this vertex to all faces
        face_distances = []
        for face_idx, face_eq in enumerate(unique_faces):
            # Skip if vertex already on this face
            if v_idx in face_eq['vertices_on_face']:
                continue
            # Distance to average plane
            dist = abs(np.dot(face_eq['normal'], vertex) + face_eq['d'])
            face_distances.append((dist, face_idx))
        
        # Sort by distance (closest first)
        face_distances.sort()
        
        # Assign to closest faces until vertex is on at least 2 faces
        faces_needed = 2 - vertex_face_count[v_idx]
        assigned_count = 0
        
        for dist, face_idx in face_distances:
            if assigned_count >= faces_needed:
                break
            # Assign to this face (no distance limit to ensure all vertices
            # get assigned to 2 faces)
            unique_faces[face_idx]['vertices_on_face'].append(v_idx)
            vertex_face_count[v_idx] += 1
            vertices_assigned += 1
            assigned_count += 1
            
            # Log if distance is large
            if dist > 1.0:
                print(f"[POLY FORM]   V{v_idx+1} assigned to F{face_idx+1} "
                      f"(dist={dist:.2f}mm)")
    
    print(f"[POLY FORM]   Assigned {vertices_assigned} vertex-face pairs")
    
    # Display updated vertex counts
    for face_idx, face_eq in enumerate(unique_faces):
        vertices_display = [v+1 for v in sorted(face_eq['vertices_on_face'])]
        print(f"[POLY FORM] F{face_idx+1:2d}: {len(face_eq['vertices_on_face']):2d} vertices: {vertices_display}")
    
    # Step 4.5: Refine face equations using least squares fit
    # This step can be repeated multiple times for iterative refinement
    print("\n[POLY FORM] Step 4.5: Refining face equations using least squares")
    
    max_refinement_iterations = 1  # Can be increased for iterative refinement
    normal_dot_tolerance = 0.001  # Normals must be nearly parallel
    d_value_tolerance = 0.1  # d-values must be within 0.1mm
    
    for iteration in range(max_refinement_iterations):
        if max_refinement_iterations > 1:
            print(f"\n[POLY FORM]   Refinement iteration {iteration + 1}/{max_refinement_iterations}")
        ...
        for face_idx, face_eq in enumerate(unique_faces):
            ...
            # Need at least 3 vertices to fit a plane
            if len(vertices_on_face) < 3:
                print(f"[POLY FORM]   F{face_idx+1}: Skipped (only {len(vertices_on_face)} vertices)")
                faces_failed += 1
                continue
            
            # Gather vertices that lie on this face
            face_vertices = selected_vertices[vertices_on_face]
            
            # Least squares plane fit: Ax + By + Cz + D = 0
            # Normalize so that A^2 + B^2 + C^2 = 1
            # Using SVD method for numerical stability
            ...
            normal_new = Vt[-1]  # Last row = normal to best-fit plane
            
            # Ensure consistent orientation with original normal
            if np.dot(normal_new, face_eq['normal']) < 0:
                normal_new = -normal_new
            
            # Compute d from centroid: normal · centroid + d = 0
            d_new = -np.dot(normal_new, centroid)
            
            # Validate against original face equation
            # Check 1: Normals must be nearly parallel
            dot_product = np.dot(normal_new, face_eq['normal'])
            normal_check = abs(abs(dot_product) - 1.0) < normal_dot_tolerance
            
            # Check 2: d-values must be close
            d_check = abs(d_new - face_eq['d']) < d_value_tolerance
            
            if normal_check and d_check:
                # Update face equation with refined values
                face_eq['normal'] = normal_new
                face_eq['d'] = d_new
                faces_refined += 1
            else:
                # Log failure for debugging
                print(f"[POLY FORM]   F{face_idx+1}: Validation failed - "
                      f"dot={dot_product:.6f}, Δd={abs(d_new - face_eq['d']):.4f}")
                faces_failed += 1
        
        print(f"[POLY FORM]   Refined: {faces_refined}, Failed: {faces_failed}")
        
        # If doing multiple iterations, re-detect vertices with refined equations
        if iteration < max_refinement_iterations - 1:
            print(f"[POLY FORM]   Re-detecting vertices with refined equations...")
            for face_idx, face_eq in enumerate(unique_faces):
                vertices_on_face = []
                
                for v_idx in range(N):
                    vertex = selected_vertices[v_idx]
                    # Use refined normal and d for range check
                    dist_P = np.dot(face_eq['normal_P'], vertex) + face_eq['d_P']
                    dist_M = np.dot(face_eq['normal_M'], vertex) + face_eq['d_M']
    ...
    # Step 4.6: Compare and merge similar face equations
    # This step can be repeated multiple times for iterative merging
    print("\n[POLY FORM] Step 4.6: Comparing and merging similar faces")
    
    max_merge_iterations = 1  # Can be increased for iterative merging
    
    # Refined tolerances (stricter than initial)
    # 1 degree: cos(1°) ≈ 0.99985, so 1 - 0.99985 = 0.00015
    refined_normal_tol = 0.00015  # ~1 degree
    refined_d_tol = 2.0  # 2mm
    d_prefilter_range = 10.0  # Pre-filter: check faces within 10 units
    
    for iteration in range(max_merge_iterations):
        if max_merge_iterations > 1:
            print(f"\n[POLY FORM]   Merge iteration {iteration + 1}/{max_merge_iterations}")
        ...
        for i in range(len(unique_faces)):
            ...
            for j in range(i + 1, len(unique_faces)):
                ...
                face_j = unique_faces[j]
                d_j = face_j['d']
                
                # Pre-filter: Only compare if d-values are within range
                if abs(d_i - d_j) > d_prefilter_range:
                    continue
                
                # Check 1: Normals must be nearly parallel
                dot_product = np.dot(face_i['normal'], face_j['normal'])
                
                # For same direction normals
                if dot_product > 0:
                    normal_check = abs(dot_product - 1.0) < refined_normal_tol
                    d_check = abs(d_i - d_j) < refined_d_tol
                else:
                    # For opposite direction normals
                    normal_check = abs(dot_product + 1.0) < refined_normal_tol
                    d_check = abs(d_i + d_j) < refined_d_tol
                
                if normal_check and d_check:
                    # Faces i and j are duplicates - merge j into i
                    print(f"[POLY FORM]   Merging F{j+1} into F{i+1}: "
                          f"dot={dot_product:.6f}, Δd={abs(d_i - d_j):.4f}")
                    
                    # Combine vertices from both faces (remove duplicates)
                    combined_vertices = list(set(
                        face_i['vertices_on_face'] + 
                        face_j['vertices_on_face']
                    ))
                    face_i['vertices_on_face'] = sorted(combined_vertices)
                    
                    # Mark face j for removal
        ...
        if faces_to_remove:
            unique_faces = [
                face for idx, face in enumerate(unique_faces)
                if idx not in faces_to_remove
            ]
            print(f"[POLY FORM]   Merged {merge_count} faces, "
                  f"{len(unique_faces)} faces remaining")
        else:
            print(f"[POLY FORM]   No faces merged in this iteration")
            break  # No changes, stop iterating
    
    # Display final face count and vertices
    print(f"\n[POLY FORM] Final face count after merging: {len(unique_faces)}")
    for face_idx, face_eq in enumerate(unique_faces):
    ...
    def build_polygons_from_face_edges(edges_on_face, vertices_on_face,
        ...
        while len(used_vertices) < len(vertices_on_face) and iteration < max_iterations:
            ...
            # Iterate interference resolution until no overlaps remain
            max_separation_iterations = 3
            for separation_iter in range(max_separation_iterations):
                print(f"  [STEP 6.4.5] Separation iteration {separation_iter + 1}/{max_separation_iterations}")
                ...
                # Find all interfering pairs
                interfering_pairs = []
                for i in range(len(shapely_polys)):
                    for j in range(i + 1, len(shapely_polys)):
                        poly_i = shapely_polys[i]['shapely']
                        poly_j = shapely_polys[j]['shapely']
                        
                        try:
                            intersection = poly_i.intersection(poly_j)
                            if intersection.area > 1e-6:
                                interfering_pairs.append((i, j, intersection.area))
                                print(f"  [STEP 6.4.5] Interference found: poly {i} and {j}, "
                                      f"overlap area={intersection.area:.2f}")
                        except:
                            pass
                
                if not interfering_pairs:
                    print(f"  [STEP 6.4.5] No interference found in iteration {separation_iter + 1}")
                    break
                
                print(f"  [STEP 6.4.5] Found {len(interfering_pairs)} interfering pair(s), resolving...")
                print(f"  [STEP 6.4.5] Found {len(interfering_pairs)} interfering pair(s), resolving...")
                
                # Process each interfering pair using intersection/difference
                processed_indices = set()
                for i, j, overlap_area in interfering_pairs:
                    ...
                    print(f"  [STEP 6.4.5] Separating poly {i} "
                          f"({len(poly_i_data['vertices'])} verts) and poly {j} "
                          f"({len(poly_j_data['vertices'])} verts)")
                    
                    # Separate into non-overlapping regions: intersection and differences
                    try:
                        ...
                        for region_name, shapely_geom in result_regions:
                            ...
                            if len(matched_verts) >= 3:
                                # Add as new non-overlapping region
                                shapely_polys.append({
                        ...
                        print(f"  [STEP 6.4.5]   Failed to separate: {e}")
                
                # Rebuild polygon lists with non-overlapping regions for next iteration
                # Remove processed polygons and keep new regions
                all_refined_polys = []
                for idx, poly_data in enumerate(shapely_polys):
                    if idx not in processed_indices:
                        all_refined_polys.append(poly_data['vertices'])
                
                print(f"  [STEP 6.4.5] After separation: {len(all_refined_polys)} polygons for next iteration")
    ...
    # Show initial face composition before validation
        ...
        for edge, face_list in edges_in_1_face[:20]:
            ...
            if face_idx < len(unique_faces):
                ...
                if 'polygons' in face_eq and poly_idx < len(face_eq['polygons']):
                    ...
                    # Check in all_face_polygons for hole/alternate markers
    ...
    while removal_iteration < max_removal_iterations:
        ...
        for candidate_result in sorted_candidates:
            ...
            if current_poly_count <= 1:
                print(f"\n[POLY FORM]     ✗ Cannot remove face {face_label}: would leave face {candidate_face_idx+1} with 0 polygons")
                print(f"[POLY FORM]     Keeping polygon despite invalid edges (face needs at least 1 polygon)")
    ...
    # Verify no faces were left empty
    print(f"\n[POLY FORM]   Verifying face integrity...")
    for face_idx, face_eq in enumerate(unique_faces):
        poly_count = len(face_eq.get('polygons', []))
        if poly_count == 0:
            print(f"[POLY FORM]     WARNING: Face {face_idx+1} has NO polygons after validation!")
        else:
    ...
    for face_idx, face_eq in enumerate(unique_faces):
        ...
        # Keep only primary polygons (boundaries and holes) in original face
        face_eq['polygons'] = primary_polygons
        
        # Create new face for each alternate
    ...
    for face_idx, face_eq in enumerate(unique_faces):
        ...
        # Create face data for each boundary (there may be multiple if face was split)
        for boundary_poly in boundaries:
            face_data = {
