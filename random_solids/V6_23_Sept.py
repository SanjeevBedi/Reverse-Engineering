...
def build_solid_with_polygons_test(seed=47315, quiet=False):
    ...
    # Create box at (0,0,0) of size (30,40,1)
    box = BRepPrimAPI_MakeBox(30, 40, 60).Shape()
    # Move box to (10,0,0)
    trsf = gp_Trsf()
    trsf.SetTranslation(gp_Vec(10, 3, 0))
    moved_box = BRepBuilderAPI_Transform(box, trsf, True).Shape()
        ...
        for poly, _ in hidden:
            if hasattr(poly, 'exterior') and not poly.is_empty:
                ...
                for interior in poly.interiors:
                    ...
                    print(f"Shape type: {poly.geom_type}")
                    if poly.geom_type == 'Polygon':
                        coords = list(poly.exterior.coords)
                        print(f"Exterior coordinates: {coords}")
                        for i, interior in enumerate(poly.interiors):
                            print(f"Interior {i} coordinates: {list(interior.coords)}")
                    elif hasattr(poly, 'coords'):
                        coords = list(poly.coords)
                        print(f"Coordinates: {coords}")
                    elif hasattr(poly, 'geoms'):
                        for i, geom in enumerate(poly.geoms):
                            print(f"Sub-geometry {i} type: {geom.geom_type}")
                            if geom.geom_type == 'Polygon':
                                print(f"Sub-geometry {i} exterior coordinates: {list(geom.exterior.coords)}")
                                for j, interior in enumerate(geom.interiors):
                                    print(f"Sub-geometry {i} interior {j} coordinates: {list(interior.coords)}")
                            elif hasattr(geom, 'coords'):
                                print(f"Sub-geometry {i} coordinates: {list(geom.coords)}")
