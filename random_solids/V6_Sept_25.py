...
from Reconstruction.config_system import (
    ConfigurationManager, create_default_config, load_config
)
...
def plot_four_views(solid, user_normal,
    ...
    for normal, label, vertex_array in view_configs:
        ...
        # Helper to match a 2D point to all projected vertex indices within tolerance
        def find_vertex_indices(pt, tol=1e-5):
            matches = []
            pt_arr = np.array(pt)
        ...
        for poly, _ in hidden:
            if hasattr(poly, 'exterior') and not poly.is_empty:
                ...
                for i in range(len(coords)-1):
                    idxs1 = find_vertex_indices(coords[i])
                    idxs2 = find_vertex_indices(coords[i+1])
                    for idx1 in idxs1:
                        for idx2 in idxs2:
...
def main():
    ...
    plot_four_views(solid, projection_normal,
                   ...
                   Vertex_Side_View,
                   Vertex_Iso_View)
    # Accept comma-separated normal (already handled above)
