...
def build_solid_with_polygons_test(config, quiet=False):
    ...
    seed = config.seed
    print(
        f"[DEBUG] Calling build_solid_with_polygons(config, seed={seed}, "
    ...
    trsf = gp_Trsf()
    trsf.SetTranslation(gp_Vec(30, 27, 23))
    moved_box = BRepBuilderAPI_Transform(box, trsf, True).Shape()
    cut = BRepAlgoAPI_Cut(cut_shape2, moved_box)
    cut_shape1 = cut.Shape()
    box = BRepPrimAPI_MakeBox(10, 10, 45).Shape()
    trsf = gp_Trsf()
    trsf.SetTranslation(gp_Vec(25, 20, 20))
    moved_box = BRepBuilderAPI_Transform(box, trsf, True).Shape()
    cut = BRepAlgoAPI_Cut(cut_shape2, moved_box)
    cut_shape2 = cut.Shape()
...
def get_face_normal_from_opencascade(face):
    ...
    try:
        ...
        print(
            f"        Parameter bounds: U[{u_min:.3f}, {u_max:.3f}], "
        ...
        try:
            ...
            print(
                f"        Surface point: ({point.X():.3f}, {point.Y():.3f}, {point.Z():.3f})"
            )
            print(
                f"        dU vector: ({d1u.X():.3f}, {d1u.Y():.3f}, {d1u.Z():.3f})"
            )
            print(
                f"        dV vector: ({d1v.X():.3f}, {d1v.Y():.3f}, {d1v.Z():.3f})"
            )
            ...
            if normal_vec.Magnitude() > 1e-10:
                ...
                if face_orientation == TopAbs_REVERSED:
                    orientation_multiplier = -1.0
                    print("        REVERSED face - flipping derivative normal")
                ...
                normal_print = (
                    f"[{face_normal[0]:.6f}, {face_normal[1]:.6f}, {face_normal[2]:.6f}]"
                )
...
def visualize_3d_solid(solid_shape, selected_vertices=None, edges=None,
    ...
    try:
        ...
        # Create figure with space for buttons on the left
        fig = plt.figure(figsize=(18, 12))
        ax = fig.add_subplot(111, projection='3d')
        
        # Adjust subplot to make room for buttons
        plt.subplots_adjust(left=0.18, right=0.98, bottom=0.05, top=0.95)
        ...
        for i, face_data in enumerate(all_face_data):
            ...
            if len(vertices) > 2:
                vertices_closed = np.vstack([vertices, vertices[0]])
                line, = ax.plot(vertices_closed[:, 0], vertices_closed[:, 1], vertices_closed[:, 2], 
                ...
                face_scatter.append(scatter)
                
                face_center = np.mean(vertices, axis=0)
                label_text = f'F{i+1}'
                text = ax.text(face_center[0], face_center[1], face_center[2], 
                       label_text, fontsize=9, color='black', ha='center', va='center', alpha=0.7)
                face_labels.append(text)
        
        # Store selected vertices plot elements
        vertex_scatter = None
        vertex_labels = []
        ...
        if selected_vertices is not None and len(selected_vertices) > 0:
            ...
            print(f"  → Plotting {len(selected_vertices)} selected vertices "
                  f"as red spheres")
            
            # Plot vertices with smaller, semi-transparent dots
            vertex_scatter = ax.scatter(selected_vertices[:, 0],
                       selected_vertices[:, 1],
                       selected_vertices[:, 2],
                       color='red', s=60, alpha=0.7,
                       edgecolors='darkred', linewidth=1.5,
                       label=f'Selected Vertices '
                             f'({len(selected_vertices)})',
                       marker='o')
            
            # Add vertex labels with offset to avoid overlap with dots
            # Calculate a small offset based on data range
            x_range = (selected_vertices[:, 0].max() -
                       selected_vertices[:, 0].min())
            y_range = (selected_vertices[:, 1].max() -
                       selected_vertices[:, 1].min())
            z_range = (selected_vertices[:, 2].max() -
                       selected_vertices[:, 2].min())
            offset = max(x_range, y_range, z_range) * 0.015
            
            for i, vertex in enumerate(selected_vertices):
                # Place label slightly offset in x and z directions
                text = ax.text(vertex[0] + offset,
                        vertex[1],
                        vertex[2] + offset * 1.5,
                        f'V{i}',
        ...
        # Store edge plot elements
        class3_lines = []
        class2_lines = []
        class1_lines = []
        
        # Plot edges if provided with classification-based colors
        if edges_with_class is not None and len(edges_with_class) > 0:
            if selected_vertices is not None:
                print(f"  → Plotting {len(edges_with_class)} "
                      f"reconstructed edges with color-coded classifications")
                
                # Color mapping: class 3=black, 2=gray, 1=yellow
                class_colors = {3: 'black', 2: 'gray', 1: 'yellow'}
                class_alphas = {3: 0.9, 2: 0.7, 1: 0.5}
                class_widths = {3: 3, 2: 2.5, 1: 2}
                class_counts = {1: 0, 2: 0, 3: 0}
                
                for v1_idx, v2_idx, classification in edges_with_class:
                    v1 = selected_vertices[v1_idx]
                    v2 = selected_vertices[v2_idx]
                    color = class_colors.get(classification, 'pink')
                    alpha = class_alphas.get(classification, 0.5)
                    width = class_widths.get(classification, 2)
                    class_counts[classification] = (
                        class_counts.get(classification, 0) + 1)
                    
                    line, = ax.plot([v1[0], v2[0]], [v1[1], v2[1]],
                            [v1[2], v2[2]],
                            color=color, linewidth=width,
                            alpha=alpha, zorder=10)
                    
                    # Store lines by classification
                    if classification == 3:
                        class3_lines.append(line)
                ...
                for cls in [3, 2, 1]:
                    if class_counts[cls] > 0:
                        ax.plot([], [], color=class_colors[cls],
                                linewidth=class_widths[cls],
                                alpha=class_alphas[cls],
                                label=f'Class {cls} edges: '
                                      f'{class_counts[cls]} '
                                      f'({"3 views" if cls==3 else "2 views" if cls==2 else "1 view"})')
        elif edges is not None and len(edges) > 0:
            # Fallback to old behavior if only edges provided
            if selected_vertices is not None:
                print(f"  → Plotting {len(edges)} "
                      f"reconstructed edges as black lines")
        ...
        # Create axes for buttons on the left side
        button_ax = plt.axes([0.01, 0.4, 0.15, 0.25], facecolor='lightgoldenrodyellow')
        
        # Define button labels and initial states
        button_labels = [
            'Vertices (Red)',
            'Solid Polygons',
            'Class 3 (Black)',
        ...
        # Initial visibility states (all True)
        initial_states = [
            vertex_scatter is not None,
            len(face_lines) > 0,
            len(class3_lines) > 0,
            len(class2_lines) > 0,
            len(class1_lines) > 0
        ...
        def toggle_visibility(label):
            ...
            elif label == 'Solid Polygons':
                # Toggle all face elements
                current_vis = face_lines[0].get_visible() if face_lines else True
                for line in face_lines:
                    line.set_visible(not current_vis)
                for scatter in face_scatter:
                    scatter.set_visible(not current_vis)
                for text in face_labels:
                    text.set_visible(not current_vis)
            
            elif label == 'Class 3 (Black)':
                # Toggle class 3 edges
                if class3_lines:
                    current_vis = class3_lines[0].get_visible()
                    for line in class3_lines:
                        line.set_visible(not current_vis)
            
            elif label == 'Class 2 (Gray)':
                # Toggle class 2 edges
                if class2_lines:
                    current_vis = class2_lines[0].get_visible()
                    for line in class2_lines:
                        line.set_visible(not current_vis)
            
            elif label == 'Class 1 (Yellow)':
                # Toggle class 1 edges
                if class1_lines:
                    current_vis = class1_lines[0].get_visible()
                    for line in class1_lines:
                        line.set_visible(not current_vis)
...
def plot_four_views(solid, user_normal,
    ...
    for normal, label, vertex_array in view_configs:
        ...
        def process_polygon_edges(polygon_data, visibility_value):
            nonlocal edges_found, vertices_used  # Access outer scope variables
    ...
    plt.tight_layout()
    plt.savefig("PDFfiles/four_views.pdf", format="pdf")
    plt.show()
...
def main():
    ...
    # Save view summaries for debugging
    if front_view_summary is not None:
        np.save('Output/front_view_summary.npy', front_view_summary)
        print(f"[DEBUG] Saved front_view_summary.npy")
    if top_view_summary is not None:
        np.save('Output/top_view_summary.npy', top_view_summary)
        print(f"[DEBUG] Saved top_view_summary.npy")
    ...
    if front_view_summary is not None and top_view_summary is not None:
        ...
        # Save side view summary for debugging
        if side_view_summary is not None:
            np.save('Output/side_view_summary.npy', side_view_summary)
            print(f"[DEBUG] Saved side_view_summary.npy")
            print(f"\n[DEBUG] SIDE VIEW CONNECTIVITY MATRIX")
            print(f"Side view summary shape: {side_view_summary.shape}")
            print(f"Number of vertices in side view: {side_view_summary.shape[0]}")
            if side_view_summary.shape[1] > 6:
                print(f"Connectivity matrix columns: {side_view_summary.shape[1] - 6}")
                # Print ALL vertices in side view
                print(f"\nALL {side_view_summary.shape[0]} vertices in side view:")
                print(f"Format: V# | 3D coords (x,y,z) | 2D proj (y,z) | connections")
                for i in range(side_view_summary.shape[0]):
                    vertex_3d = side_view_summary[i, 0:3]
                    # Side view projects (y, z) from 3D (x, y, z)
                    proj_2d = (vertex_3d[1], vertex_3d[2])
                    connectivity = side_view_summary[i, 6:]
                    connected_to = [j for j, val in enumerate(connectivity) if val > 0]
                    print(f"  V{i:2d}: ({vertex_3d[0]:7.3f}, {vertex_3d[1]:7.3f}, {vertex_3d[2]:7.3f}) -> proj({proj_2d[0]:7.3f}, {proj_2d[1]:7.3f}) -> conn {connected_to}")
        
        if front_view_summary is not None:
            print(f"\n[DEBUG] FRONT VIEW CONNECTIVITY MATRIX")
            print(f"Front view summary shape: {front_view_summary.shape}")
            print(f"Number of vertices in front view: {front_view_summary.shape[0]}")
            if front_view_summary.shape[1] > 6:
                print(f"Connectivity matrix columns: {front_view_summary.shape[1] - 6}")
                # Print ALL vertices in front view
                print(f"\nALL {front_view_summary.shape[0]} vertices in front view:")
                print(f"Format: V# | 3D coords (x,y,z) | 2D proj (x,z) | connections")
                for i in range(min(10, front_view_summary.shape[0])):
                    vertex_3d = front_view_summary[i, 0:3]
                    # Front view projects (x, z) from 3D (x, y, z)
                    proj_2d = (vertex_3d[0], vertex_3d[2])
                    connectivity = front_view_summary[i, 6:]
                    connected_to = [j for j, val in enumerate(connectivity) if val > 0]
                    print(f"  V{i:2d}: ({vertex_3d[0]:7.3f}, {vertex_3d[1]:7.3f}, {vertex_3d[2]:7.3f}) -> proj({proj_2d[0]:7.3f}, {proj_2d[1]:7.3f}) -> conn {connected_to}")
        ...
        if side_view_summary is None:
            ...
            # === ADDITIONAL VALIDATION: EDGE CONNECTIVITY ===
            # Filter out spurious vertices that don't have valid edge connections
            # A real vertex must have edges connecting to other vertices in the view summaries
            print("\n" + "="*60)
            print("EDGE CONNECTIVITY VALIDATION")
            print("="*60)
            print("Goal: Filter out fake vertices without valid edge connections")
            print("Method: Check if vertex has edges in at least 2 views")
            
            def count_edges_for_vertex(vertex, view_summary, match_indices):
                """
                Count how many edges this vertex has in a view summary.
                
                Args:
                    vertex: 3D coordinates [x, y, z]
                    view_summary: Summary array for the view
                    match_indices: Which coordinates to match (e.g., [0,1] for x,y)
                
                Returns:
                    int: Number of edges found for this vertex
                """
                edge_count = 0
                tolerance_edge = 1e-4
                
                # Find row indices matching this vertex
                matching_rows = []
                for i in range(view_summary.shape[0]):
                    world_coords = view_summary[i, 0:3]
                    coord_proj = [world_coords[idx] for idx in match_indices]
                    vertex_proj = [vertex[idx] for idx in match_indices]
                    
                    if np.allclose(coord_proj, vertex_proj, atol=tolerance_edge):
                        matching_rows.append(i)
                
                # Count non-zero connectivity values for these rows
                for row_idx in matching_rows:
                    if row_idx < view_summary.shape[0]:
                        # Check connectivity columns (starting from column 6)
                        connectivity_row = view_summary[row_idx, 6:]
                        edge_count += np.count_nonzero(connectivity_row > 0)
            ...
            for i, vertex in enumerate(selected_vertices):
                ...
                top_edges = count_edges_for_vertex(vertex, top_view_summary, [0, 1])  # x,y
                front_edges = count_edges_for_vertex(vertex, front_view_summary, [0, 2])  # x,z
                side_edges = count_edges_for_vertex(vertex, side_view_summary, [1, 2])  # y,z
                
                # A vertex is valid if it has edges in at least 2 views
                # This filters out spurious vertices from degenerate polygons
                views_with_edges = sum([top_edges > 0, front_edges > 0, side_edges > 0])
                
                if i < 5 or vertex[0] == 35.0 and abs(vertex[1] - 28.875) < 0.001 and abs(vertex[2] - 16.470) < 0.001:
                    print(f"  V{i+1}: ({vertex[0]:.3f}, {vertex[1]:.3f}, {vertex[2]:.3f})")
                    print(f"      Top edges: {top_edges}, Front edges: {front_edges}, Side edges: {side_edges}")
                    print(f"      Views with edges: {views_with_edges}/3 → {'VALID' if views_with_edges >= 2 else 'REJECTED'}")
                
                if views_with_edges >= 2:
                    validated_vertices.append(vertex)
            
            selected_vertices = np.array(validated_vertices)
            print(f"\nSelected vertices (after edge validation): {len(selected_vertices)}")
        ...
        if len(selected_vertices) > 0:
            output_filename = "Output/selected_vertices.txt"
            ...
            valid_edges, edges_with_class = reconstruct_edges_from_views(
                selected_vertices,
                top_view_summary,
                front_view_summary,
                side_view_summary,
            ...
            # Save edges to file with classifications
            if len(valid_edges) > 0:
                edge_filename = "Output/reconstructed_edges.txt"
                with open(edge_filename, 'w') as f:
                    f.write("Reconstructed 3D Solid Edges\n")
                    f.write("="*50 + "\n")
                    f.write(f"Total edges: {len(valid_edges)}\n")
                    f.write("Format: V1_idx -- V2_idx (class)\n")
                    f.write("Classes: 1=one view, 2=two views, 3=three views\n\n")
                    for i, (v1_idx, v2_idx, classification) in enumerate(edges_with_class):
                        v1 = selected_vertices[v1_idx]
                        v2 = selected_vertices[v2_idx]
                        f.write(f"Edge {i+1:2d}: V{v1_idx:2d} -- V{v2_idx:2d} "
                                f"(class={classification})\n")
                        f.write(f"  Start: ({v1[0]:8.3f}, {v1[1]:8.3f}, "
                                f"{v1[2]:8.3f})\n")
                        f.write(f"  End:   ({v2[0]:8.3f}, {v2[1]:8.3f}, "
                                f"{v2[2]:8.3f})\n")
            ...
            # Save to text file
            summary_txt = "Output/unified_summary.txt"
            ...
            summary_npy = "Output/unified_summary.npy"
