    ...
    if polygon.geom_type == 'Polygon':
        if outline_only:
            ...
            if facecolor != 'none':
                ...
                # Draw outline only when facecolor is 'none'
                x, y = polygon.exterior.xy
                ax.plot(x, y, color=edgecolor, linestyle=linestyle, linewidth=linewidth, label=label)
...
def extract_wire_vertices_in_sequence(wire, wire_id):
    ...
    The orientation is a hierarchical property in OpenCASCADE that applies at multiple levels:
    
    Face orientation - determines if the face normal points inward or outward
    Edge orientation within wire - determines edge direction in the wire loop
    Vertex orientation within edge - determines which vertex is "start" vs "end" in the edge's parametric direction
    ...
    try:
        ...
        if TOPEXP_AVAILABLE:
            print(f"            ✓ TopExp available - using enhanced orientation method")
            try:
                ...
                vertex_chain = []
                vertex_orientations = []  # Store orientation info for each vertex
                
                for edge_num, edge in enumerate(edges):
                    edge_num += 1  # 1-based numbering for display
                    
                    try:
                        # ENHANCED FIX: Use vertex orientations within edges for proper polygon development
                        ...
                        orientation_str = "FORWARD" if edge_orientation == TopAbs_FORWARD else "REVERSED" if edge_orientation == TopAbs_REVERSED else str(edge_orientation)
                        
                        # Get vertices with proper orientation consideration
                        from OCC.Core.TopoDS import TopoDS_Vertex
                        vertex_start = TopoDS_Vertex()
                        vertex_end = TopoDS_Vertex()
                        topexp_Vertices(topods.Edge(edge), vertex_start, vertex_end, True)  # True = use orientation
                        
                        # CRITICAL: Get vertex orientations within the edge
                        vertex_start_orient = vertex_start.Orientation()
                        vertex_end_orient = vertex_end.Orientation()
                        
                        vertex_start_orient_str = "FORWARD" if vertex_start_orient == TopAbs_FORWARD else "REVERSED" if vertex_start_orient == TopAbs_REVERSED else str(vertex_start_orient)
                        vertex_end_orient_str = "FORWARD" if vertex_end_orient == TopAbs_FORWARD else "REVERSED" if vertex_end_orient == TopAbs_REVERSED else str(vertex_end_orient)
                        ...
                        print(f"            Edge {edge_num}: {orientation_str}")
                        print(f"              Start Vertex: ({start_coords[0]:.1f},{start_coords[1]:.1f},{start_coords[2]:.1f}) [{vertex_start_orient_str}]")
                        print(f"              End Vertex: ({end_coords[0]:.1f},{end_coords[1]:.1f},{end_coords[2]:.1f}) [{vertex_end_orient_str}]")
                        
                        # Apply edge orientation within wire to determine vertex order
                        if edge_orientation == TopAbs_REVERSED:
                            ...
                            vertex_start_orient_str, vertex_end_orient_str = vertex_end_orient_str, vertex_start_orient_str
                            print(f"              REVERSED edge - swapped vertices")
                        
                        # FIXED CONNECTIVITY: Only add start vertex for first edge, always add end vertex
                        if edge_num == 1:
                            # First edge: add both start and end vertices
                            vertex_chain.append(start_coords)
                            vertex_orientations.append(f"E{edge_num}:V-{vertex_start_orient_str[:3]}")
                            print(f"              First edge: added start vertex with orientation {vertex_start_orient_str}")
                            
                            vertex_chain.append(end_coords)
                            vertex_orientations.append(f"E{edge_num}:V-{vertex_end_orient_str[:3]}")
                            print(f"              First edge: added end vertex with orientation {vertex_end_orient_str}")
                        else:
                            # Subsequent edges: only add end vertex (start should connect to previous end)
                            # Check if start connects to previous end
                            previous_end = vertex_chain[-1]
                            start_distance = np.linalg.norm(np.array(start_coords) - np.array(previous_end))
                            
                            # Use more realistic tolerance for edge connectivity (1e-6 instead of 1e-10)
                            if start_distance < 1e-6:
                                # Perfect connectivity - just add end vertex
                                vertex_chain.append(end_coords)
                                vertex_orientations.append(f"E{edge_num}:V-{vertex_end_orient_str[:3]}")
                                print(f"              Edge {edge_num}: connected properly, added end vertex")
                            else:
                                # Connectivity gap - check if it's due to orientation/precision issues
                                # Try connecting to end vertex instead of start
                                end_distance = np.linalg.norm(np.array(end_coords) - np.array(previous_end))
                                
                                if end_distance < 1e-6:
                                    # End vertex connects - add start vertex instead
                                    vertex_chain.append(start_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{vertex_start_orient_str[:3]}")
                                    print(f"              Edge {edge_num}: end connects to previous, added start vertex")
                                else:
                                    # True connectivity gap - add both start and end (this indicates an issue)
                                    print(f"              Edge {edge_num}: WARNING - connectivity gap start:{start_distance:.2e} end:{end_distance:.2e}")
                                    vertex_chain.append(start_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{vertex_start_orient_str[:3]}")
                                    vertex_chain.append(end_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{vertex_end_orient_str[:3]}")
                        ...
                        # Fallback to old method if orientation fails - still try to get vertex orientations
                        try:
                            from OCC.Core.TopoDS import TopoDS_Vertex
                            vertex1 = TopoDS_Vertex()
                            vertex2 = TopoDS_Vertex()
                            ...
                            # Try to get vertex orientations even in fallback
                            try:
                                v1_orient = vertex1.Orientation()
                                v2_orient = vertex2.Orientation()
                                v1_orient_str = "FORWARD" if v1_orient == TopAbs_FORWARD else "REVERSED" if v1_orient == TopAbs_REVERSED else str(v1_orient)
                                v2_orient_str = "FORWARD" if v2_orient == TopAbs_FORWARD else "REVERSED" if v2_orient == TopAbs_REVERSED else str(v2_orient)
                            except:
                                v1_orient_str = "UNKNOWN"
                                v2_orient_str = "UNKNOWN"
                            ...
                            print(f"            Edge {edge_num}: FALLBACK method")
                            print(f"              Vertex1: ({v1_coords[0]:.1f},{v1_coords[1]:.1f},{v1_coords[2]:.1f}) [{v1_orient_str}]")
                            print(f"              Vertex2: ({v2_coords[0]:.1f},{v2_coords[1]:.1f},{v2_coords[2]:.1f}) [{v2_orient_str}]")
                            
                            if edge_num == 1:
                                vertex_chain.extend([v1_coords, v2_coords])
                                vertex_orientations.extend([f"E{edge_num}:V-{v1_orient_str[:3]}", f"E{edge_num}:V-{v2_orient_str[:3]}"])
                            else:
                                # Check connectivity
                                previous_end = vertex_chain[-1]
                                v1_distance = np.linalg.norm(np.array(v1_coords) - np.array(previous_end))
                                v2_distance = np.linalg.norm(np.array(v2_coords) - np.array(previous_end))
                                
                                if v1_distance < v2_distance:
                                    vertex_chain.append(v2_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{v2_orient_str[:3]}")
                                else:
                                    vertex_chain.append(v1_coords)
                                    vertex_orientations.append(f"E{edge_num}:V-{v1_orient_str[:3]}")
                ...
                if len(vertex_chain) > 2:
                    ...
                    if first_tuple == last_tuple:
                        ...
                        vertex_orientations = vertex_orientations[:-1]  # Remove corresponding orientation
                        print(f"            Removed closing duplicate vertex")
                
                # DIAGNOSTIC: Temporarily disable ALL duplicate removal to see raw vertices
                # Face 3 should have 6 vertices from 6 edges - let's see what we actually get
                cleaned_vertices = vertex_chain
                cleaned_orientations = vertex_orientations
                print(f"            RAW VERTEX CHAIN (no duplicate removal): {len(vertex_chain)} vertices")
                
                vertex_chain = cleaned_vertices
                vertex_orientations = cleaned_orientations
                print(f"            After duplicate removal: {len(vertex_chain)} vertices")
                
                vertices = vertex_chain
                print(f"            ✓ Built vertex chain: {len(vertices)} vertices")
                # Print vertices with DETAILED ORIENTATION FLAGS (edge + vertex orientations)
                if vertices:
                    # Include detailed orientation information for each vertex coordinate
                    vertex_coords_with_orient = []
                    for i, v in enumerate(vertices):
                        orient_info = vertex_orientations[i] if i < len(vertex_orientations) else "UNKNOWN"
                        vertex_coords_with_orient.append(f"({v[0]:.1f},{v[1]:.1f},{v[2]:.1f})[{orient_info}]")
                    
                    vertex_coords = " → ".join(vertex_coords_with_orient)
                    print(f"            VERTEX ORDER WITH DETAILED ORIENTATIONS:")
                    print(f"            {vertex_coords}")
                    
                    # Count edge and vertex orientations for summary
                    forward_edges = len([e for e in edges if e.Orientation() == TopAbs_FORWARD])
                    reversed_edges = len([e for e in edges if e.Orientation() == TopAbs_REVERSED])
                    print(f"            EDGE ORIENTATIONS: {forward_edges} FORWARD, {reversed_edges} REVERSED")
                    print(f"            VERTEX ORIENTATIONS: Check individual vertex tags above")
...
def plot_arrays_visualization(array_A, array_B, array_C, unit_projection_normal):
    ...
    # Plot array_C polygons first (background) with dashed black lines
    for i, poly_data in enumerate(array_C):
        try:
            polygon = poly_data['polygon']
            name = poly_data['name']
            ...
            if polygon.geom_type == 'Polygon' and polygon.area > 0:
                plot_polygon(polygon, ax3, facecolor='none', edgecolor='black', 
                           alpha=1.0, linewidth=0.8, linestyle='--', label=f'C: {name}')
        except:
            pass
    
    # Plot array_B polygons on top (foreground) with solid black lines
    for i, poly_data in enumerate(array_B):
        try:
            polygon = poly_data['polygon']
            name = poly_data['name']
            ...
            if polygon.geom_type == 'Polygon' and polygon.area > 0:
                plot_polygon(polygon, ax3, facecolor='none', edgecolor='black', 
                           alpha=1.0, linewidth=0.8, linestyle='-', label=f'B: {name}')
        except:
            pass
...
def classify_faces_by_projection(face_polygons, unit_projection_normal):
    ...
    for i, polygon_data in enumerate(face_polygons):
        ...
        # Project face to 2D polygon
        try:
            projected_vertices = project_face_to_projection_plane(outer_boundary, unit_projection_normal)
            ...
            if polygon.area > 1e-6:
                ...
                print(f"  → Added Face_{face_id} (area: {polygon.area:.2f})")
...
def process_solid_with_projection_normal(solid_shape, unit_projection_normal):
    """
    Process a solid with a given projection normal and return arrays B and C.
    
    Args:
        solid_shape: OpenCASCADE solid shape
        unit_projection_normal: Normalized projection vector [x, y, z]
    
    Returns:
        tuple: (array_B, array_C) where:
            - array_B: List of visible face polygons
            - array_C: List of hidden face polygons and intersections
    """
    print("\n" + "="*60)
    print("PROCESSING SOLID WITH PROJECTION NORMAL")
    print("="*60)
    ...
    if solid_shape is None:
        print("✗ Invalid solid shape provided")
        return [], []
    
    # Ensure projection normal is normalized
    unit_projection_normal = np.array(unit_projection_normal, dtype=float)
    magnitude = np.linalg.norm(unit_projection_normal)
    if magnitude < 1e-10:
        print("✗ Invalid projection normal (zero vector)")
        return [], []
    
    unit_projection_normal = unit_projection_normal / magnitude
    print(f"Normalized projection normal: [{unit_projection_normal[0]:.6f}, {unit_projection_normal[1]:.6f}, {unit_projection_normal[2]:.6f}]")
    
    try:
        # Step 1: Extract faces from the solid
        print("\nStep 1: Extracting faces from solid...")
        face_polygons = extract_faces_from_solid(solid_shape)
        ...
        print(f"✓ Extracted {len(face_polygons)} faces from solid")
        
        # Step 2: Classify faces using the projection normal
        print("\nStep 2: Classifying faces with historic algorithm...")
        array_A, array_B, array_C = classify_faces_by_projection(face_polygons, unit_projection_normal)
        ...
        print("="*60)
        print(f"Input: {len(face_polygons)} faces")
        print(f"Array B (visible): {len(array_B)} polygons")
        print(f"Array C (hidden + intersections): {len(array_C)} polygons")
        ...
        print(f"Total area B: {total_area_B:.2f}")
        print(f"Total area C: {total_area_C:.2f}")
        print("="*60)
        
        return array_B, array_C
        
    except Exception as e:
        print(f"✗ Error processing solid: {e}")
        traceback.print_exc()
        return [], []

def test_solid_processing():
    """Test the solid processing function with projection normal [1,1,1]."""
    print("\n" + "="*80)
    print("TESTING SOLID PROCESSING WITH PROJECTION NORMAL [1,1,1]")
    print("="*80)
    ...
    try:
        # Step 1: Create a test solid
        print("\nStep 1: Creating test solid...")
        solid_shape = create_opencascade_solid()
        
        if solid_shape is None:
            print("✗ Failed to create test solid")
            return
        
        print("✓ Test solid created successfully")
        
        # Step 2: Analyze the solid geometry
        print("\nStep 2: Analyzing solid geometry...")
        analyze_solid_geometry(solid_shape)
        
        # Step 3: Test with projection normal [1,1,1]
        test_projection_normal = [1, 1, 1]
        print(f"\nStep 3: Testing with projection normal {test_projection_normal}...")
        
        array_B, array_C = process_solid_with_projection_normal(solid_shape, test_projection_normal)
        ...
        print(f"Array B (visible faces): {len(array_B)} polygons")
        for i, poly_data in enumerate(array_B):
            name = poly_data.get('name', f'Polygon_{i}')
            area = poly_data['polygon'].area if hasattr(poly_data.get('polygon'), 'area') else 0
            print(f"  B{i+1}: {name} (area: {area:.2f})")
        
        print(f"\nArray C (hidden + intersections): {len(array_C)} polygons")
        for i, poly_data in enumerate(array_C):
            name = poly_data.get('name', f'Polygon_{i}')
            area = poly_data['polygon'].area if hasattr(poly_data.get('polygon'), 'area') else 0
            print(f"  C{i+1}: {name} (area: {area:.2f})")
        
        # Step 5: Create visualization
        print(f"\nStep 5: Creating visualization...")
        unit_projection_normal = np.array(test_projection_normal, dtype=float)
        unit_projection_normal = unit_projection_normal / np.linalg.norm(unit_projection_normal)
        
        plot_arrays_visualization([], array_B, array_C, unit_projection_normal)
